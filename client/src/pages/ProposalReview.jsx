  ...
  return (
    <div className="dashboard">
      <div className="container">
        ...
        {accepted && (
          <PaymentPanel job={job} acceptedProposal={accepted} onChange={load} />
        )}

        {accepted && job.hiredFreelancer && (
            ...
            {proposals.map(p => (
              <div key={p.id} className="card proposal-card">
                <div className="proposal-card-head">
                  <Link to={`/freelancers/${p.freelancer?.id}`} className="proposal-freelancer">
                    <Avatar user={p.freelancer} size={42} />
                    <div>
                      <div className="proposal-freelancer-name">{p.freelancer?.name}</div>
                      <div className="muted small">
                        {p.freelancer?.title || 'Freelancer'}
                        {p.freelancer?.rating ? ` · ⭐ ${p.freelancer.rating}` : ''}
                      </div>
                    </div>
                  </Link>
